const Raining = (() => {
  ...
  function RainyDay(options) {
    ...
    const src = typeof options.image === 'string'
    ...
    if (src.tagName.toLowerCase() === 'img') {
      ...
      const self = this;
      const style = src.currentStyle || window.getComputedStyle(src, false);
  ...
  RainyDay.prototype.initialize = function (options) {
    const sourceParent = this.imgSource
      || options.parentElement
      || document.getElementsByTagName('body')[0];
    const parentOffset = window.getOffset(sourceParent);
